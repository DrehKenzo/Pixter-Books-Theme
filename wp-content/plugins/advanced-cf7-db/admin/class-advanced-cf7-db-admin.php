...
class Advanced_Cf7_Db_Admin {
	...
	function vsz_cf7_plugin_menu(){
		...
		add_menu_page( "Advanced CF7 DB", "Dados do Form.", $cap, "contact-form-listing", array($this,"vsz_contact_form_callback"), 'dashicons-list-view	' , 45);
		///// Menu pages for Import CSV
		add_submenu_page( 'contact-form-listing', __('Import CSV', 'advanced-cf7-db'), __('Importar em CSV', ' advanced-cf7-db'), $cap, 'import_cf7_csv',array($this,'vsz_import_cf7_csv') );
	...
	function vsz_cf7_after_bulkaction_btn_callback($fid){
		...
		<select id="vsz-cf7-export" name="vsz-cf7-export" data-fid="<?php echo $fid; ?>">
			<option value="-1"><?php _e('Exportar para...'); ?></option>
	...
	function vsz_cf7_after_datesection_btn_callback($fid){
		...
		?><input value="<?php echo ((isset($_POST['search_cf7_value'])) && !empty($_POST['search_cf7_value']) ? htmlspecialchars(stripslashes(sanitize_text_field($_POST['search_cf7_value']))) : ''); ?>" type="text" class="" id="cf7d-search-q" name="search_cf7_value" placeholder="<?php echo _e('Digite alguma coisa...'); ?>" id="" />
	...
	function vsz_cf7_admin_after_body_edit_field_func($form_id, $row_id){
		...
		$row_id = (int)$row_id;
		?><td>
			<a data-rid="<?php echo $row_id; ?>" href="#TB_inline?width=600&height=550&inlineId=cf7d-modal-edit-value" id="cf7d-edit-form" class="thickbox cf7d-edit-value" name="Editar informações">
	...
	function vsz_cf7_after_admin_edit_values_form_callback($form_id){
		...
		?><div class="cf7d-modal" id="cf7d-modal-edit-value" style="display:none;">
			<form action="" class="cf7d-modal-form loading" id="cf7d-modal-form-edit-value" method="POST">
            	<div class="popup-note"><span>*(Tipo de campo)</span></div>
				...
				<div class="cf7d-modal-footer">
					...
					<input type="submit" id="update_cf7_value" name="vsz_cf7_save_field_value" value="Salvar alterações" class="button button-primary button-large" />
				</div>
			</form>
			<!------------------------------------ Ajax loader ----------------------------------------->
